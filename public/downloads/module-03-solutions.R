...
# Complete module: 7 lessons, all worked examples and exercise solutions.
...
# SETUP
# Install once, if needed: install.packages(c("dplyr", "readxl", "tidyr", "writexl"))
library(dplyr)
library(readxl)
library(tidyr)
library(writexl)
...
# WORKED EXAMPLE: Read Excel files
# Load the package
library(readxl)

# Read the Incomes worksheet and save it as df
df <- read_excel(income_excel_file, sheet = "Incomes")
...
# WORKED EXAMPLE
# Save df as a CSV file, without an extra column of row names
write.csv(df, file = "incomes_saved.csv", row.names = FALSE)

# Show the folder where R saved the file
getwd()

# WORKED EXAMPLE: Save an Excel file with write_xlsx()
# Load the package
library(writexl)
...
# EXERCISE SOLUTION: Save the data in both formats
